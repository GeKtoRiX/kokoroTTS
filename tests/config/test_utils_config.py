...
def test_load_config_reads_env_and_sets_duplicate_limits(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("APP_STATE_PATH", str(tmp_path / "data" / "state.json"))
    ...
    assert config.app_state_path == os.path.abspath(str(tmp_path / "data" / "state.json"))
...
def test_load_config_for_duplicate_space_has_no_char_limit(monkeypatch):
    ...
    assert Path(config.app_state_path).as_posix().endswith("data/app_state.json")
