...
from kokoro_tts.ui.features.audio_player_feature import AudioPlayerFeature
...
def test_ui_common_helpers_cover_export_signature_and_modes():
    ...
    assert (
        ui_common.runtime_mode_from_flags(tts_only_enabled=True) == ui_common.RUNTIME_MODE_DEFAULT
    )
    assert (
        ui_common.runtime_mode_from_flags(tts_only_enabled=False)
        == ui_common.RUNTIME_MODE_TTS_MORPH
    )
...
def test_main_module_guard_executes_launch(monkeypatch):
    called = {"launch": False}
    monkeypatch.setitem(
        sys.modules, "app", SimpleNamespace(launch=lambda: called.__setitem__("launch", True))
    )
...
def test_open_path_in_file_manager_requires_existing_folder(tmp_path: Path):
    with pytest.raises(FileNotFoundError):
        AudioPlayerFeature._open_path_in_file_manager(tmp_path / "missing")


def test_open_path_in_file_manager_uses_resolved_opener_on_linux(monkeypatch, tmp_path: Path):
    folder = tmp_path / "records"
    folder.mkdir()

    launched: list[list[str]] = []
    monkeypatch.setattr("kokoro_tts.ui.features.audio_player_feature.sys.platform", "linux")
    monkeypatch.setattr(
        "kokoro_tts.ui.features.audio_player_feature.shutil.which",
        lambda name: "/usr/bin/xdg-open" if name == "xdg-open" else None,
    )
    monkeypatch.setattr(
        "kokoro_tts.ui.features.audio_player_feature.subprocess.Popen",
        lambda command: launched.append(list(command)),
    )

    AudioPlayerFeature._open_path_in_file_manager(folder)
    assert launched == [["/usr/bin/xdg-open", str(folder.resolve())]]


def test_open_path_in_file_manager_raises_when_opener_missing(monkeypatch, tmp_path: Path):
    folder = tmp_path / "records"
    folder.mkdir()

    monkeypatch.setattr("kokoro_tts.ui.features.audio_player_feature.sys.platform", "linux")
    monkeypatch.setattr(
        "kokoro_tts.ui.features.audio_player_feature.shutil.which", lambda _name: None
    )

    with pytest.raises(FileNotFoundError, match="xdg-open"):
        AudioPlayerFeature._open_path_in_file_manager(folder)
