...
def ui_app(tmp_path: Path):
    ...
    app_instance = create_tkinter_app(
        ...
        export_morphology_sheet=lambda dataset, file_format="ods": (str(tmp_path / f"{dataset}.{file_format}"), "ok"),
    ...
    app_instance._threaded = lambda work, on_success=None: on_success(work()) if on_success is not None else work()
...
def test_runtime_language_and_mix_interactions(ui_app, monkeypatch):
    ...
    assert app_instance._language_code_from_display("???") == app_instance.default_lang
    assert app_instance._language_display_from_code("unknown").endswith(f"({app_instance.default_lang})")
    ...
    app_instance.mix_enabled_var.set(True)
    app_instance._on_mix_toggle()
    app_instance._set_mix_listbox_values(app_instance.current_voice_ids, selected=app_instance.current_voice_ids[:1])
    ...
    app_instance.accordion_setters["custom"] = lambda expanded: app_instance.generate_status_var.set(str(expanded))
...
def test_pronunciation_morphology_and_stream_handlers(ui_app, monkeypatch):
    ...
    app_instance.export_morphology_sheet = lambda dataset, file_format: (str(tmp_path / f"{dataset}.{file_format}"), "ready")
    ...
    app_instance.export_morphology_sheet = lambda dataset: (str(tmp_path / f"{dataset}.ods"), "ready2")
    app_instance._on_export_morphology()
    ...
    app_instance.morphology_db_view = lambda dataset, limit, offset: (
        {"headers": ["token_text", "lemma", "upos"], "value": [["hello", "hello", "NOUN"], ["run", "run", "VERB"]]},
        "ok",
    )
...
def test_audio_player_interactions_and_state(ui_app, monkeypatch):
    ...
    opened = {"called": False}
    monkeypatch.setattr(app_instance, "_on_audio_player_open", lambda autoplay=False: opened.__setitem__("called", bool(autoplay)))
    ...
    seek_targets = []
    monkeypatch.setattr(app_instance, "_audio_player_seek_vlc_ms", lambda ms: seek_targets.append(ms))
    ...
    monkeypatch.setattr(
        app_instance,
        "_load_audio_file_async",
        lambda path, *, autoplay, history_index=None, resume_seconds=None: next_calls.append((path, autoplay, history_index)),
    )
    ...
    assert app_instance._is_text_input_widget(SimpleNamespace(winfo_class=lambda: "Button")) is False
    assert app_instance._is_text_input_widget(None) is False

    app_instance.audio_player_volume_var.set(1.1)
    result = app_instance._on_audio_shortcut_volume(SimpleNamespace(widget=SimpleNamespace(winfo_class=lambda: "Button")), 0.1)
    assert result == "break"
    assert app_instance._on_audio_shortcut_volume(SimpleNamespace(widget=text_widget), 0.1) is None
    app_instance.audio_player_loaded_path = audio_path
    assert app_instance._on_audio_shortcut_seek(SimpleNamespace(widget=SimpleNamespace(winfo_class=lambda: "Button")), 1.0) == "break"
    app_instance.audio_player_is_playing = True
    assert app_instance._on_audio_shortcut_play_pause(SimpleNamespace(widget=SimpleNamespace(winfo_class=lambda: "Button"))) == "break"
...
def test_layout_helpers_close_and_thread_timer_branches(ui_app, monkeypatch):
    ...
    widget = app_instance._add_labeled_widget(helper_parent, "Label", lambda parent: ttk.Entry(parent))
    ...
    tkapp_mod.TkinterDesktopApp._threaded(app_instance, lambda: "ok", lambda value: started.append(value))
    assert started == ["ok"]
    ...
    monkeypatch.setattr(app_instance, "_save_audio_player_state", lambda: called.__setitem__("saved", called["saved"] + 1))
    monkeypatch.setattr(app_instance, "_stop_generate_timer", lambda: called.__setitem__("stopped", called["stopped"] + 1))
    monkeypatch.setattr(app_instance, "_stop_audio", lambda preserve_player_position=False: called.__setitem__("stopped", called["stopped"] + 1))
    monkeypatch.setattr(app_instance, "_audio_player_cancel_tick", lambda: called.__setitem__("tick", called["tick"] + 1))
    monkeypatch.setattr(app_instance, "_audio_player_release_vlc", lambda: called.__setitem__("released", called["released"] + 1))
...
def test_audio_file_loading_stream_and_tick_branches(ui_app, monkeypatch):
    ...
    tkapp_mod.sf = _SfOk()
    app_instance._load_audio_file_async(audio_path, autoplay=True, history_index=3, resume_seconds=1.25)
    ...
    try:
        ...
        played = {"count": 0}
        app_instance._on_audio_player_play = lambda: played.__setitem__("count", played["count"] + 1)
    ...
    monkeypatch.setattr(tkapp_mod, "VlcAudioBackend", lambda: (_ for _ in ()).throw(RuntimeError("vlc-init-fail")))
    assert app_instance._ensure_vlc_player() is False
    ...
    scheduled = {"count": 0}
    app_instance._audio_player_schedule_tick = lambda: scheduled.__setitem__("count", scheduled["count"] + 1)
    app_instance._on_audio_player_tick()
    ...
    app_instance.audio_player_total_frames = 0
    app_instance.audio_player_seek_scale = ttk.Scale(root, from_=0, to=1, variable=tk.DoubleVar(master=root))
    ...
    try:
        ...
        app_instance.generate_all = lambda **_kwargs: (_ for _ in ()).throw(RuntimeError("stream-fail"))
        app_instance._on_stream_start()
...
def test_audio_additional_edge_branches(ui_app, monkeypatch):
    ...
    try:
        ...
        app_instance._update_audio_player_buttons = lambda: None
        after_calls = []
        app_instance.root = SimpleNamespace(after=lambda delay, callback: after_calls.append(delay) or "after-id")
        app_instance.audio_player_sample_rate = 100
        ...
        app_instance.vlc_audio = SimpleNamespace(set_time_ms=lambda _ms: (_ for _ in ()).throw(RuntimeError("seek-fail")))
        app_instance._audio_player_seek_vlc_ms(100)
        assert any("seek vlc" in message.lower() for message in logger.exceptions)

        app_instance.vlc_audio = SimpleNamespace(set_pause=lambda _on: (_ for _ in ()).throw(RuntimeError("pause-fail")))
        tkapp_mod.sd = SimpleNamespace(stop=lambda: (_ for _ in ()).throw(RuntimeError("sd-pause-fail")))
        app_instance.audio_player_backend = "sounddevice"
        app_instance.audio_player_is_playing = True
        app_instance.audio_player_sample_rate = 100
        app_instance.audio_player_total_frames = 100
        ...
        app_instance.root = SimpleNamespace(after_cancel=lambda _job: (_ for _ in ()).throw(RuntimeError("cancel-fail")))
        ...
        tkapp_mod.vlc = SimpleNamespace(State=SimpleNamespace(Ended="ended", Stopped="stopped", Error="error"))
        app_instance._audio_player_try_auto_next = lambda: False
        app_instance._audio_player_update_progress = lambda: None
        app_instance._update_audio_player_buttons = lambda: None
        app_instance._save_audio_player_state = lambda: None
        ...
        app_instance.audio_player_volume_var.set(1.1)
        app_instance.vlc_audio = SimpleNamespace(set_volume=lambda _value: (_ for _ in ()).throw(RuntimeError("volume-fail")))
        app_instance._save_audio_player_state = lambda: None
        ...
        app_instance._audio_player_rebuild_waveform(np.asarray([[0.1, -0.1], [0.2, -0.2]], dtype=np.float32))
        assert app_instance.audio_player_waveform is not None

        app_instance._audio_player_total_seconds = original_total_seconds
        app_instance.vlc_audio = SimpleNamespace(get_length_ms=lambda: (_ for _ in ()).throw(RuntimeError("len-fail")))
        ...
        app_instance.vlc_audio = SimpleNamespace(get_time_ms=lambda: (_ for _ in ()).throw(RuntimeError("time-fail")))
        app_instance.audio_player_backend = "sounddevice"
        app_instance.audio_player_is_playing = True
        app_instance.audio_player_sample_rate = 100
        ...
        headers, rows = app_instance._project_morphology_preview_rows("lexemes", {"headers": ["token_text"], "value": "bad"})
        assert headers == ["token_text", "lemma", "upos"]
        assert rows == []
        headers, rows = app_instance._project_morphology_preview_rows("unknown", {"headers": ["h"], "value": ["v"]})
        assert headers == ["h"]
        assert rows == [["v"]]

        preview = app_instance._build_pos_table_preview_from_lexemes({"headers": ["x"], "value": ["bad"]})
        assert preview["headers"] == ["No data"]
        preview = app_instance._build_pos_table_preview_from_lexemes(
            {"headers": ["lemma", "upos"], "value": [["", ""], ["hello", "NOUN"], ["hello", "NOUN"], ["x", "ZZZ"]]}
        )
