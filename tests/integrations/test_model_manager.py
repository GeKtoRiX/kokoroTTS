...
def test_model_manager_skips_pronunciation_rules_without_lexicon(monkeypatch):
    class FakeModel:
        def __init__(self, repo_id):
            self.repo_id = repo_id
    ...
    manager = ModelManager(
        ...
        logger=logger,
        pronunciation_rules={"e": {"OpenAI": "o"}},
    )
    pipeline = manager.get_pipeline("ef_demo")
    assert isinstance(pipeline.g2p, SimpleNamespace)
    assert any("Skipping pronunciation lexicon overrides" in line for line in logger.debugs)
