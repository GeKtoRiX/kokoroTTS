...
def test_runtime_registration_adds_custom_language_and_voice():
    ...
    try:
        registered = register_runtime_voices(
            "q",
            voices=[("Custom Demo", "q_demo")],
            language_label="Custom Language",
            aliases=("q-custom",),
        )
        assert "q_demo" in registered
        assert normalize_lang_code("q-custom") == "q"
        custom_choices = get_voice_choices("q")
        assert ("Custom Demo", "q_demo") in custom_choices
        assert "q" in available_language_codes()
        mixed = normalize_voice_input("q_demo", voice_mix=["q_demo", "af_heart"])
        assert mixed == "q_demo"
    finally:
