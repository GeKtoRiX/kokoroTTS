...
def test_normalize_numbers_phone_like_digit_sequence():
    text = "Call [tnumber]0123411 or [tnumber=1234567]."
    expected = "Call oh one two three four double one or one two three four five six seven."
    assert app.normalize_numbers(text) == expected


def test_normalize_numbers_year_like_values():
    text = "Years: [date]1993, [date=2006], [date]2024."
    expected = "Years: nineteen ninety three, two thousand and six, twenty twenty four."
    assert app.normalize_numbers(text) == expected
...
def test_smart_split_max_chars():
    ...
    assert app.smart_split(text, max_chars=12, keep_sentences=False) == [
        "Hello world.",
        "Hi there.",
    ]
