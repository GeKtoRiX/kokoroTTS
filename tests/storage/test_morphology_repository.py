...
def test_morphology_repository_inserts_expressions_table(tmp_path: Path):
    ...
    connection = sqlite3.connect(db_path)
    try:
        expression_count = connection.execute("SELECT COUNT(*) FROM morph_expressions").fetchone()[
            0
        ]
...
def test_morphology_repository_keeps_legacy_extra_tables(tmp_path: Path):
    ...
    try:
        connection.execute(
            'CREATE TABLE IF NOT EXISTS "morph_reviews" ("id" INTEGER PRIMARY KEY, "status" TEXT)'
        )
