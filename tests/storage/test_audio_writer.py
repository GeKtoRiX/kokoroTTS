...
def test_audio_writer_save_wav_and_build_paths(tmp_path, monkeypatch):
    ...
    assert Path(single_path[0]).parent.parent.name == "records"
    assert re.fullmatch(r"\d{2}-\d{2}-\d{2}", Path(single_path[0]).parent.name)
    assert all(Path(path).parent.parent.name == "records" for path in multi_paths)
    assert all(re.fullmatch(r"\d{2}-\d{2}-\d{2}", Path(path).parent.name) for path in multi_paths)
...
def test_audio_writer_save_text_sidecar(tmp_path):
    logger = _Logger()
    writer = AudioWriter(str(tmp_path), sample_rate=24000, logger=logger)
    audio_path = tmp_path / "sample.wav"
    audio_path.write_bytes(b"audio")

    txt_path = writer.save_text_sidecar(str(audio_path), "hello\nworld")

    assert txt_path.endswith(".txt")
    assert Path(txt_path).is_file()
    assert Path(txt_path).read_text(encoding="utf-8") == "hello\nworld"
